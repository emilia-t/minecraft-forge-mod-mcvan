...
public class VideoScreenRenderer implements BlockEntityRenderer<VideoScreenBlockEntity> {
    ...
    private static final int TEXTURE_WIDTH = 256;  // 4 blocks * 64px
    private static final int TEXTURE_HEIGHT = 192; // 3 blocks * 64px

    private static final float RENDER_WIDTH = 4.0f;  // 4 blocks wide
    private static final float RENDER_HEIGHT = 3.0f; // 3 blocks tall

    public VideoScreenRenderer(BlockEntityRendererProvider.Context context, ResourceLocation dynamicTextureId) {
    ...
    public void render(
        ...
        try {
            ...
            Direction dir = be.getBlockState().getValue(VideoScreenBlock.FACING);// 调整位置和旋转
            switch (dir) {
                case NORTH -> {}
            ...
            float viewScreenX = (float) ScreenDynamicTexture.getViewScreenX();// 渲染边框
            float viewScreenY = (float) ScreenDynamicTexture.getViewScreenY();
            if(viewScreenX == 0 || viewScreenY==0){//材质未更新
                return;
            }
            else{
                float videoWidthBlocks = viewScreenX / VideoData.blockPixelDefaultF;// 计算视频在方块坐标系中的位置和尺寸
                float videoHeightBlocks = viewScreenY / VideoData.blockPixelDefaultF;//2.25
                float startX = (RENDER_WIDTH - videoWidthBlocks) / 2.0f;//0
                float startY = (RENDER_HEIGHT - videoHeightBlocks) / 2.0f;
                float endX = startX + videoWidthBlocks;//4
     ...
     */
    @Override
    public boolean shouldRenderOffScreen(VideoScreenBlockEntity entity) {
    ...
    private void renderBorder(
            VideoScreenBlockEntity pBlockEntity,
            Matrix4f pPose,
            MultiBufferSource bufferSource,
            float videoStartX,
            float videoStartY,
     ...
     * @param pConsumer
     */
    private void renderScreen(VideoScreenBlockEntity pBlockEntity, Matrix4f pPose, VertexConsumer pConsumer) {
     ...
     * @param pDirection
     */
    private void renderFace(VideoScreenBlockEntity pBlockEntity, Matrix4f pPose, VertexConsumer pConsumer,
                            float pX0,
                            float pX1,
                            float pY0,
                            float pY1,
