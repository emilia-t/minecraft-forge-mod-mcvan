 ...
 * c2s 客户端发送请求
 */
public class VideoBlockFrameIndexNextPacket {
    private final BlockPos pos;

    public VideoBlockFrameIndexNextPacket(BlockPos pos) {
        this.pos = pos;
    }

    public VideoBlockFrameIndexNextPacket(FriendlyByteBuf buf) {
        ...
        ctx.get().enqueueWork(() -> {
            ...
            if (player != null) {
                ...
                if (be instanceof VideoScreenBlockEntity) {
                    ...
                    if(index >= store.getVideoData(url).getFrameCount() -1){//判断帧数是否合理
                        return;
                    }
